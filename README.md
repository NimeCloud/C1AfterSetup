...
- **AuthKit integration** — deploys the AuthKit authorization framework (users, groups, permissions) as App_Code + data types. On first use the code-defined groups/permissions are seeded automatically, and the **first C1 user** who logs in becomes an AuthKit **shadow user** auto-joined into the `System.Administrators` group — giving that user full AuthKit administration rights so the system is never left without an administrator.
