        ...
        altEditor.prototype,
        {
            ...
            _setup: function () {
                ...
                var modal = '<div class="modal fade altEditor-modal reveal" id="' + modal_id + '" tabindex="-1" role="dialog" data-reveal>' +
                    ...
                    // Bootstrap 5
                    '<button style="margin: initial;" type="button" class="btn-close" data-bs-dismiss="modal" aria-label="' + this.language.modalClose + '">' +
                    '</button>' +
            ...
            _openDeleteModal: function () {
                ...
                var fill = function () {
                    ...
                    var btns = '<button type="button" data-content="remove" class="btn btn-default button secondary" data-bs-dismiss="modal">' + that.language.modalClose + '</button>' +
                        '<button type="submit"  data-content="remove" class="btn btn-danger button" id="deleteRowBtn">' + that.language.delete.button + '</button>';
            ...
            completeColumnDefs: function () {
                ...
                for (var i in dt.context[0].aoColumns) {
                    ...
                    columnDefs[i] = {
                        ...
                        inlineLabel: (obj.inlineLabel !== undefined ? obj.inlineLabel : true) // YENİ EKLENDİ
                    }
                }
            ...
            */
               createDialog: function(columnDefs, modalTitle, buttonCaption, closeCaption, buttonClass, formName) {
                ...
                for (var j in columnDefs) {
                    ...
                    // --- YENİ EKLENEN ÖZEL KOD BLOĞU ---
                    // Bizim özel, etiketi solda olan checkbox durumumuzu ele al
                    else if (columnDefs[j].type.indexOf("checkbox") >= 0 && columnDefs[j].inlineLabel === false) {
                        data += "<div class='row mb-3' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) + "'>";
                        data += "    <label class='col-md-4'>" + title + ":</label>";
                        data += "    <div class='col-md-8 d-flex align-items-center'>";
                        data += "        <input class='form-check-input' "
                             + fillAttrs(columnDefs[j], ['type', 'name', 'readonly', 'disabled', 'required'])
                             + "id='" + this._quoteattr(columnDefs[j].name) + "' "
                             + "value='true'>";
                        data += "    </div>";
                        data += "</div>";
                    ...
                    // --- YENİ KOD BLOĞU SONU ---
                        ...
                        // Kütüphanenin geri kalan tüm orijinal davranışları bu blokta
                        data += "<div class='mb-3 row' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) +"'>";
                        data += "    <label for='" + this._quoteattr(columnDefs[j].name) + "' class='col-md-4 col-form-label'>" + title + ":</label>";
                        ...
                        if (columnDefs[j].type.indexOf("select") >= 0) {
                            ...
                            if (optionsArray.length > 0) {
                                for (var i = 0; i < optionsArray.length; i++) { options += "<option value='" + this._quoteattr(optionsArray[i]) + "'>" + optionsArray[i] + "</option>"; }
                            } else {
                                for (var x in optionsArray) { options += "<option value='" + this._quoteattr(x) + "'>" + optionsArray[x] + "</option>"; }
                            }
                            data += "<select class='form-select" + (columnDefs[j].select2 ? ' select2' : '') + "' "
                        ...
                        // Orijinal Checkbox Davranışı (inlineLabel: true ise çalışır)
                        else if (columnDefs[j].type.indexOf("checkbox") >= 0) {
                            data += "<div class='form-check'>";
                            data += "<input class='form-check-input' "
                                + fillAttrs(columnDefs[j], ['type', 'name', 'readonly', 'disabled', 'required'])
                                + "id='" + this._quoteattr(columnDefs[j].name) + "' value='true'>";
                            if (columnDefs[j].inlineLabel) {
                                data += "<label class='form-check-label' for='" + this._quoteattr(columnDefs[j].name) + "'>" + title + "</label>";
                            }
                            data += "</div>";
                        }
                        // Diğer tüm input'lar
                ...
                var fill = function () 
                {
                    var btns = '<button type="button" data-content="remove" class="btn btn-secondary" data-bs-dismiss="modal">' + closeCaption + '</button>'
